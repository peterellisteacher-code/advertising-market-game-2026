...
- Optional, fail-closed Image Lab. It remains globally disabled pending compatible written minor-use approval and, when activated, still opens only after the supervising teacher authorises an individual pair's short-lived session. The teacher can close it immediately.
