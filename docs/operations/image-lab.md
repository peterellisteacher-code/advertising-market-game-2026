...
| Realistic advertisement | `make-it-real-advertisement-v2` | `openai/gpt-image-2/edit` | one 1024×576 complete-ad reference, exact `{ width: 1280, height: 720 }`, high quality, one PNG | exact 1280×720 PNG |

The endpoints do not share one generic browser payload. Object Forge sends a closed set of object choices. Product realisation retains its historical product-and-scene request. Assignment Sandbox advertisement realisation sends a distinct `mode: "advertisement"` request containing only the prepared canvas, a bounded snapshot of the product details and Advertisement AIDA plan, one **Professional finish** choice and an ordered subset of **Improve** choices. Both edit operations use server-owned `prompt`, `image_urls`, `image_size`, `quality`, `num_images` and `output_format` fields. The production adapter uses an explicit `{ width, height }` object, not a named aspect-ratio preset.

The whole-ad creative controls are stable typed IDs, not free text. `finish` must be exactly one of `photographic-campaign`, `clean-product`, `lifestyle`, `premium-editorial` or `bold-poster`. `improvements` must be a duplicate-free subset of `lighting-shadows`, `materials-texture`, `background-polish`, `colour-contrast` and `depth-focus` in that canonical order. The server rejects unknown IDs, duplicates, wrong ordering, excess entries and unexpected fields before reserving allowance or dispatching to fal, and maps each accepted ID to a fixed server-owned prompt sentence. The selections are request-local in the browser and are included in the submission fingerprint, so a changed selection is a new idempotent job.

The realistic-advertisement profile is pinned and does not follow the adult-only `IMAGE_LAB_REALISE_PROFILE_ID` A/B selector. Its prompt treats every student field as literal data, preserves the supplied composition, product, colours, marks, existing wording and every existing person, and prohibits invented brands, claims, people, text, watermarks and signatures; the requested finish and improvements never override those preservation rules. The generated result is a new selected full-canvas layer; one Undo restores the prior editable design. Students are warned to check every word because image models can alter lettering. Job tokens issued under the earlier `make-it-real-advertisement-v1` profile remain readable at the same model and 1280×720 dimensions.
