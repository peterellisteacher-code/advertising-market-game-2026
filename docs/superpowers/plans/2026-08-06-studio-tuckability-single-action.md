...
- **C — Godot surfaces:** lobby staging, HUD chip + tuck conformance, RunPanel audit. GodotIQ is wired locally (addon port 6105) but deliberately NOT shipped: the addon, its rule files and `godot/.godotiq.json` are gitignored, and the `[editor_plugins]` block the wiring added to `godot/project.godot` stays uncommitted — never stage that hunk, so the public tree (and the CI Linux export built from it) stays addon-free.
