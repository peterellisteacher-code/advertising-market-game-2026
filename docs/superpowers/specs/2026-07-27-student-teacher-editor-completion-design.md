...
## 25. Current-state classification at `d15c3275`

This classification is based on the current source, not on presumed behaviour. `Present and verify` means that the required foundation exists but still needs the named runtime evidence. `Change` means that related behaviour exists but does not meet the requirement. `New` means that the required surface or authority boundary is absent.

| Acceptance requirement | Classification | Current evidence and implementation consequence |
| --- | --- | --- |
| Public `/student` without teacher controls | **New** | There is no pathname router. The single account gate currently exposes `Create a pair login` and its teacher setup code beside student login. Add route-aware shells and remove signup/reset controls from the student surface. |
| Protected `/teacher` using `!Y10English!` | **New** | There is no teacher session, dashboard or teacher-only function group. Add server-verified teacher authentication and a bounded secure cookie. |
| Typed account passwords and password replacement | **Change + new** | The server signup contract already accepts arbitrary passwords from 8 to 128 bytes. The form currently creates a 20-character password in a read-only field. Make creation editable and add a teacher-only password-replacement operation. |
| Isolated teacher playtest and factory reset | **New** | Practice mode exists, but there is no reserved teacher account/profile or dashboard return path. Add an isolated identity and apply the existing reset primitives only to that identity. |
| Teacher reset of a selected student pair | **Change** | A complete signed-in self-reset pipeline already deletes progress, drafts, assets and cloud state while preserving username/password. It currently appears to the signed-in pair and confirms with `RESET`. Reuse its idempotent cleanup machinery behind teacher authorization and target the selected pair. |
| Teacher-controlled Image Lab allowances | **Change** | Current state supports bounded configuration, pair-bound capability cookies, compare-and-swap state, idempotent job reservation and separate object/final counts. Authority is granted by a shared teacher code and counts are session-scoped rather than account-ledger records. Replace only the authority and ledger layer; preserve provider validation and idempotent job handling. |
| No teacher code on student devices | **Change** | `ImageLabPanel` explicitly renders `Teacher code` and `Wake Image Lab`. Remove both and load the authenticated pair's server allowance directly. |
| Twelve varied starter products | **Change** | The certified product-kit pack contains exactly three kits: reusable tumbler, flat-screen television and compact carry case. The generated offline catalogue already contains many reviewed assets that can supply validated single-piece starters. |
| Visually correct kit sockets | **Change** | Connector and raster-matrix tests exist, but they validate transforms rather than visible contact. Peter's carry-case screenshot contradicts visual correctness. Repair metadata/assets and add rendered contact evidence. |
| Visible undoable object deletion | **Change** | `ObjectCommandService.remove`, Delete/Backspace and accessible Layers-list delete buttons already exist and use editor history. Add the missing contextual action for the selected canvas object and preserve the existing command path. |
| Proportional draggable split pane | **Change** | The workspace uses fixed `64px 288px 1fr` columns and a binary collapsed state. `Hide library` is absolutely positioned over the drawer. Replace the fixed/collapse controller with an accessible separator while retaining the current tool rail and canvas. |
| Per-section Fill | **New using present primitives** | Semantic product shells can already recolour named regions, and generated raster assets already carry reviewed body masks. Catalogue tinting currently occurs at placement and does not provide post-placement connected-region selection. Add the bounded section-selection interaction and persistence without replacing the existing mask validation. |
| Reliable and understandable logo insertion | **Change** | Valid words plus a selected symbol already produce a logo through tested add/replace callbacks. The action is silently disabled when either prerequisite is missing and cannot focus the missing control. Treat the reported failure first as a prerequisite-discoverability reproduction, then fix any callback defect if the browser falsifies that hypothesis. |
| Explained Art Director and Strategist roles | **Change** | Stage-specific prompts, contribution counters and working role swap exist. The compact strip does not define the distinction or explain what swapping changes, and no permanent Role guide exists. Add explanation without changing the role model. |
| Complete linked instructions | **Present and change** | The web studio already has state-driven `Now`, `Why`, `Done`, `Next` guidance and a full instruction dialog; Godot contains a 23-step linked inductive argument. Update both for the new account route, teacher-controlled Image Lab, expanded editor and exact role definitions, then complete the visual-layout pass. |
| Market completion survives reload | **Present and verify** | Price insertion now keeps charged price, visible text and evidence aligned; publication failures map to bounded student messages; proof point, final review and gallery scoring exist. The Claude deployment predates some of this work. A current hosted finish-line replay is still required before the blocker can be closed. |
| A reused device can sign out and accept another pair | **Present and change** | Logout already locks local account surfaces, clears account-scoped state and waits for server logout. The earlier passwordless harness could automatically recreate the QA session. Verify against real pair login and ensure `/student` remains signed out after reload. |
| All Claude findings resolved | **Mixed** | Safe publication errors, revision-copy removal in the ordinary sync status, reset machinery, accessibility mirror and typed initial room-join errors are present. The finish line, restored-save revision text, polling error distinction, market keyboard traversal, selection parity, price done state and visible keyboard hint still need focused evidence or change. |
| Objective copy gates on final corpus | **Future release gate** | Corpus extraction and professional-language contracts exist, and prior evidence remains valid for unchanged strings. The combined changes create a new corpus; run Plain Language once, then Claude Scrubber MICROCOPY once, only after the text is stable. |
| Full deterministic and browser verification | **Future release gate** | The project already has serialized tests, build contracts, an exact-artifact verifier, CI Godot export and draft deployment tooling. Reuse these pipelines on the final integrated inputs and add only the new focused tests and browser paths. |
| Updated operations guide | **Change** | The existing private guide records the former site-wide visitor gate, teacher setup code and credential workflow. Replace those operational instructions after the new routes and controls are verified. |
| Controlled release without incidental mutations | **Present and extend** | Manifest-bound draft deployment and production-freeze practices already exist. Add explicit teacher-gate, Supabase migration and visitor-gate transition checks before any controlled external change. |
