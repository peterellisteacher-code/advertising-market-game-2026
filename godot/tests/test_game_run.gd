...
func run() -> bool:
	...
	assert(_legacy_pitch_snapshot_migrates_with_agency_state())
...
func _pitch_snapshot_round_trips_identity_phase_and_ready_levels() -> bool:
	...
	assert(snapshot.get("contract") == "pitch-run@2")
	assert(snapshot.get("phase") == "sell")
	assert(snapshot.get("teamAlias") == "Neon Narwhals")
	assert(snapshot.get("sessionId") == "local-session")
	assert(snapshot.get("teamId") == "local-team")
	assert(snapshot.get("readyLevels") == ["invent", "sell"])
	assert(Dictionary(snapshot.get("agency")).get("contract") == "agency-run@1")
...
func _legacy_pitch_snapshot_migrates_with_agency_state() -> bool:
	var game := GameRun.new()
	...
	var migrated: Dictionary = game.pitch_snapshot()
	assert(migrated.get("contract") == "pitch-run@2")
	assert(
		Dictionary(migrated.get("agency")).get("currentObjectiveId")
		== "shape-message"
	)

	var before: Dictionary = migrated.duplicate(true)
	var invalid: Dictionary = migrated.duplicate(true)
	var invalid_agency: Dictionary = Dictionary(invalid.get("agency")).duplicate(true)
	invalid_agency["activeRole"] = "chief-disruption-officer"
	invalid["agency"] = invalid_agency
