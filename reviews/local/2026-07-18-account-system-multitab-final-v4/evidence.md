# Advertising Market account system — local pre-migration gate v4
...
- Same-origin HttpOnly cookies are shared across browser tabs. A tab admitted as
  account A must never operate as account B after another tab replaces the cookie.
- Cloud campaign JSON contains arbitrary nested and top-level Fabric passthrough
  data. It may contain neither transient browser object URLs nor local assets that
  the recovery path cannot deterministically restore.
...
6. Login/signup/logout publish only a random cross-tab storage nonce. Peers lock,
   ...
   publish. Header binding remains the hard boundary if the event lags or fails.
...
9. Browser and Netlify enforce the full bounded canonical offline campaign,
   ...
   only selected top-level constraints as server-role-only defence in depth; it
...
11. Durable local assets form a restore-safe bijection: every local descriptor
    has exactly one matching cloud descriptor and an exact semantic Fabric
    `local-blob:` source; every local source anywhere in Fabric is required to be
    the same parsed object instance selected by the semantic map for its objectId,
    as well as having the matching descriptor pair. A passthrough object cannot
    borrow a tracked object's identifiers to evade recovery targeting.
...
- `web/src/domain/campaign-semantic-objects.ts` and its test
- `netlify/functions/account-session.mts` and its test
- `netlify/functions/account-progress.mts` and its test
- `netlify/functions/account-assets.mts` and its test
- `netlify/functions/lib/account-primitives.ts` and its test
- `netlify/functions/lib/account-progress-document.ts` and its test
- `netlify/functions/lib/account-assets.ts` and its test
- `netlify/functions/lib/netlify-account-assets.ts` and its test
- `netlify/functions/lib/account-backend.ts` and its test
...
- Regression was observed red before the semantic-target identity fix, then the
  parser suite passed: 15/15.
- Strict `tsc --noEmit`: exit 0.
- Focused account/domain/Netlify/main integration: 38 files, 624 tests passed.
- Full serial Vitest after the current change: 116 files, 1,795 tests passed in
  129.34 seconds.
- Build contracts: 60/60 passed.
- The existing Godot 4.7.1 game/Creator/Market gate was previously exit 0 with an
  explicit pass; this TypeScript-only change does not alter Godot sources.
