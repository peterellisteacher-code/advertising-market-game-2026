...
export const AGENCY_RUN_CONTRACT = "agency-run@1" as const;
...
const agencyRoles = ["art-director", "strategist"] as const;
const agencyStations = [
  "reception",
  "client-briefing",
  "strategy-room",
  "art-studio",
...
const agencyEvidence = z.strictObject({
  decision: z.string().trim().min(1).max(400),
  effect: z.string().trim().min(30).max(400)
});

export const AgencyRunSnapshotSchema = z.strictObject({
  contract: z.literal(AGENCY_RUN_CONTRACT),
  currentObjectiveId: z.string().trim().min(1).max(80),
  currentStationId: z.enum(agencyStations),
  activeRole: z.enum(agencyRoles),
  completedMissionIds: z.array(z.enum(requiredMissions)).max(requiredMissions.length),
  completedSidequestIds: z.array(z.enum(sidequests)).max(sidequests.length),
  evidenceByMission: z.record(z.string(), agencyEvidence),
  handoffCount: safeInteger.max(10_000),
  guideTucked: z.boolean(),
  orientationAcknowledged: z.boolean(),
  audioSettings: z.strictObject({
    enabled: z.boolean(),
    musicEnabled: z.boolean(),
    sfxEnabled: z.boolean(),
    masterVolume: z.number().finite().min(0).max(1)
  }),
  pitchSettings: z.strictObject({
    formatId: z.enum(["billboard", "magazine", "vertical-screen"]),
    animationId: z.enum(["immediate", "reveal", "slide", "spotlight", "sequence"])
  }),
  started: z.boolean()
}).superRefine((snapshot, context) => {
  const orderedSubsets: Array<{
    path: "completedMissionIds" | "completedSidequestIds";
    values: readonly string[];
    allowed: readonly string[];
  }> = [
    {
      path: "completedMissionIds",
      values: snapshot.completedMissionIds,
      allowed: requiredMissions
    },
    {
      path: "completedSidequestIds",
      values: snapshot.completedSidequestIds,
      allowed: sidequests
    }
  ];
  for (const { path, values, allowed } of orderedSubsets) {
    let priorIndex = -1;
    for (const value of values) {
      ...
      if (index <= priorIndex) {
        context.addIssue({
          code: "custom",
          path: [path],
          message: `${path} must be an ordered set`
        });
        break;
      }
      priorIndex = index;
  ...
  for (const missionId of Object.keys(snapshot.evidenceByMission)) {
    if (!snapshot.completedMissionIds.includes(
      missionId as (typeof requiredMissions)[number]
    )) {
      context.addIssue({
        code: "custom",
        path: ["evidenceByMission", missionId],
        message: "Mission evidence requires a completed mission"
      });
    }
  }
  if (new TextEncoder().encode(JSON.stringify(snapshot)).byteLength > 16_384) {
    context.addIssue({
      code: "custom",
      path: [],
      message: "Agency progress exceeds the 16 KiB checkpoint limit"
    });
  }
});

export type AgencyRunSnapshotV1 = z.infer<typeof AgencyRunSnapshotSchema>;
...
export const LocalPracticeCheckpointSchema = z.strictObject({
  ...
  pitch: AgencyRunSnapshotSchema.optional()
...
const saveProgressPayload = z.strictObject({
  checkpoint: PracticeCheckpointTokenSchema,
  pitch: AgencyRunSnapshotSchema,
  operationId
});
...
export type PracticeSaveProgressInput = z.infer<typeof saveProgressPayload>;
...
export interface PracticeRunHandler {
  ...
  saveProgress(input: PracticeSaveProgressInput): MaybePromise<LocalPracticeRecoveryV1>;
